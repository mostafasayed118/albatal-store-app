...
class CheckoutPage extends StatelessWidget {
  const CheckoutPage({super.key, CheckoutRepository? checkoutRepository, CheckoutCubit? checkoutCubit})
  ...
  Widget build(BuildContext context) {
    ...
    final consumer = BlocConsumer<CheckoutCubit, CheckoutState>(
        listener: (context, s) {
          if (s.status == CheckoutStatus.placing && s.hasPendingOrder) {
            final email = SupabaseConfig.currentUser?.email ?? 'customer@example.com';
          ...
          return Scaffold(
            ...
            body: ListView(
              ...
              children: [
                ...
                Card(
                  ...
                  child: Padding(
                    ...
                    child: Column(
                      ...
                      children: [
                        Text(l.shippingAddress, style: Theme.of(context).textTheme.titleLarge),
                ...
                const SizedBox(height: 24),
                if (s.hasAddress) ...[
                  Text(l.reviewOrder, style: Theme.of(context).textTheme.titleLarge),
                  const SizedBox(height: 8),
                  ...
                  Card(
                    ...
                    child: Padding(
                      ...
                      child: Column(
                        crossAxisAlignment: CrossAxisAlignment.start,
                        children: [
                          Text('Server-confirmed totals', style: Theme.of(context).textTheme.titleSmall),
                          const SizedBox(height: 8),
                          _ServerTotalRow(label: 'Subtotal', value: s.serverSubtotal),
                          _ServerTotalRow(label: 'Shipping', value: s.serverShipping),
              ...
              child: FilledButton(
                ...
                onPressed: s.hasAddress && !isCreating
                    ? () {
                        ...
                        context.read<CheckoutCubit>().createPendingOrder(cartItems: cart.items);
                      }
                    : null,
                child: isCreating
                    ? SizedBox(
    ...
    if (_checkoutCubit != null) {
      return BlocProvider<CheckoutCubit>.value(value: _checkoutCubit, child: consumer);
    }
    ...
    return Padding(
      ...
      child: Row(
        children: [
          ...
          const Spacer(),
          Text(value?.format() ?? '--', style: const TextStyle(fontWeight: FontWeight.w500)),
        ],
      ),
    );
  }
