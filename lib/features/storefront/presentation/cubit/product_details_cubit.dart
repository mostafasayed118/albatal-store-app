...
final class DetailsState extends Equatable {
  const DetailsState({this.color = 'Emerald', this.length = '2m', this.quantity = 1});

  final String color;
  final String length;
  final int quantity;

  DetailsState copyWith({String? color, String? length, int? quantity}) => DetailsState(
...
final class ProductDetailsCubit extends Cubit<DetailsState> {
  ...
  void quantity(int value) => emit(state.copyWith(quantity: value.clamp(1, 99).toInt()));
}
