...
- [ ] **0.2 Staging DB password (blocks Tasks 2, 3, 4, 5):** reset via Supabase dashboard on `zvpjngdgbpnkkqrorkul`, then set in each execution shell: `$env:STAGING_DB_URL = 'postgresql://postgres.zvpjngdgbpnkkqrorkul:<password>@aws-0-eu-west-1.pooler.supabase.com:5432/postgres'`. Never echo, never commit.
