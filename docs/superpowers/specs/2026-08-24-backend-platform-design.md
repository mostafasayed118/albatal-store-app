...
- Upload: `StorageService.uploadProductImage(productId, file)` → `product-images/{productId}/{uuid}.{ext}` `upsert:false` then `admin_set_product_images` to register. Cache lifetime: **one year**, set at upload time as `FileOptions.cacheControl = StorageService.productImageCacheSeconds` (`'31536000'`). Supabase Storage INTERPOLATES that field into a `max-age=` prefix (`supabase/storage`, `src/storage/uploader.ts`), so the value must BEGIN with a duration — the earlier `public, max-age=31536000, immutable` literal was not sendable and would have been stored as the malformed `Cache-Control: max-age=public, max-age=31536000, immutable`. Directives appended AFTER the duration do pass through (`'31536000, immutable'` → `Cache-Control: max-age=31536000, immutable`), so the `immutable` in this spec was reachable; it was dropped deliberately, because reaching it means relying on the server echoing a field it documents as a bare second count, for a directive with no effect on a mobile client that does its own disk caching. A year is safe because the object path is fresh per upload and never upserted, so a cached URL cannot go stale; `uploadAvatar` deliberately keeps the SDK's one-hour default because its path IS reused per user. Scope: this governs the `/object/…` url — `/render/image/…` responses proxy imgproxy and carry that service's cache headers, not this metadata.
