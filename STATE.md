...
- **NOT verified: the test suite.** `flutter test` could not run in this session (incident above), so the 1,097-test expectation set is unrun locally; CI is the verifier. README's test badge stays at the last VERIFIED number (1,090) until a green suite run — do not bump it on the strength of a count alone.
