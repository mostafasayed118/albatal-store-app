...
- Owner ask: "review and fix" `deliverables/gstack/pre-launch-check-albatal-store-2026-09-25.md`. L1 first: all 9 blockers re-verified live on `master` @ `ead60c2` (admin double-reads-minor-as-major `admin_mappers.dart:166`; reviews select missing `author_name` `:26-31,92-102` vs view-only column `057:70-95`; public-bucket photo upload `:83-90`; comment-only 058 fabric/checkout deltas; `run_all_migrations.sql` last marker 013 + `GRANT … TO PUBLIC` `:1263`; checkout `rate_limit_take` only in `validate_coupon` `066:358`; missing `allowBackup`/cleartext). Owner then approved **lib + protected paths** L2 scope via explicit prompt.
- L2 worktree: `.trees/fix-server-audit` (baseline `ead60c2`; `master` source untouched — only this `STATE.md` record modified there). Audit of the uncommitted part-68/69/71 batch confirmed the blocking set already present: B1 minor-unit boundary (`Money`, `tryParseMajor`, admin pages/stores), B2/B3 `product_reviews_public` read + `submit_product_review` RPC submit, B4 private `review-images` bucket (069), B5 `serverMeteredCheckoutEnabled=false` gate, B6 regen (combined file last marker now 073 + CI drift check), B7 072 bounds/throttle, B9 manifest hardening, payment `paymobCard` handshake. Generated `linux/macos/windows` plugin-registrant noise reverted (`diff --stat` for those paths now empty).
- Only new code this run (R12, the one open item): `lib/shared/services/deep_link_parser.dart` — post-decode guard rejects empty / `/`-containing / >256-char product ids (kills the `%2F`→`/` vector in both https and custom-scheme shapes; `Routes.product` re-encodes anyway, this is defense-in-depth) + regression test in `deep_link_parser_test.dart`.
- Evidence (worktree, CI-pinned toolchain): `flutter analyze --no-pub` → **No issues found**; targeted deep-link **9/9 passed**; full `flutter test --no-pub` → **1,133 passed** (the `[SMOKE] FAIL admin_dashboard_data` line is the harness's intentional negative fixture); `dart format` (3.13.4) on the 2 touched files → 0 changed; `git diff --check` clean. Default-Flutter `test` startup still hits the recorded cross-SDK kernel mismatch (expected 130, found 138) — CI-pinned 3.47.5 used instead. Verifier: **APPROVE**.
- No commit, push, or merge performed (needs human approval). Remaining owner gates: review/merge `fix/server-audit` (incl. 068–073) → apply to staging → execute the 44-case checklist against live Supabase (R12 now covered by unit test; B5/B7/R6 still need live-schema proof); rotate `sbp_` token (standing item).

## New — 2026-09-25 (part 72: GStack pre-launch readiness check)

- Owner ask: comprehensive pre-launch check across code review, security audit and QA testing. Report-only; no source file modified.
- Scope: shipping branch `master` @ `ead60c2`; 369 `lib/` Dart files, 197 `test/` Dart files, 68 committed migrations, 10 Edge Functions, 3 CI workflows.
- Method: three parallel read-only specialist subagents (code review, OWASP+STRIDE security, QA) with `file:line` evidence, plus lead-side independent verification of the shared findings.
- **Headline: the part-68/69/71 remediation is NOT on the shipping branch.** `git rev-list master..fix/server-audit` = 0; both `fix/server-audit` and `fix/audit-all` point at `ead60c2` = `master`, with the fixes present only as uncommitted working-tree changes. All five part-70 critical findings remain live on `master`.
- Verified by the lead directly: `scripts/run_all_migrations.sql` ends at migration 013 and re-issues `GRANT EXECUTE ON FUNCTION create_checkout_order TO PUBLIC`; `admin_mappers.dart:166` reads the minor-unit INTEGER as a double (100x mischarge); `supabase_reviews_repository.dart:26-31,92-102` selects `author_name` from the base table, but that column exists only on the `product_reviews_public` view (`057:11-21` vs `:70-95`); review photos upload to the public `product-images` bucket (`:83-90`).
- New: `057_product_reviews.sql` is itself labelled "REVIEW-GATED PROPOSAL — Owner review required before applying", so the reviews feature was built against a migration that is explicitly gated.
- Merged findings: 18 (9 blocking / 4 medium / 4 low). Blocking: admin 100x price; reviews read broken; review submit orphan + permanently blocked retries; review photos in a public admin-only bucket; sample/metered checkout not server-authoritative; stale migration script granting checkout to PUBLIC; unbounded checkout RPC; 068-073 unmerged; Android `allowBackup`/cleartext hardening missing.
- QA: 44-case checklist + 10-row exception matrix produced. CI evidence at `ead60c2`: **1,101/1,101 tests, 74.2% coverage** (run `35912608176`), analyzer **0 diagnostics / 566 files**. Local `flutter test` still blocked by the host pipe incident (`CreateFile failed 231`) — no local result claimed.
- **Key insight for the record: green CI cannot see any of these defects.** CI never executes SQL migrations, RLS, or device/E2E flows, so the blocking set is invisible to it.
- Release posture: **No-Go (Fail)** on `master` @ `ead60c2`; converts to conditional Go once 068-073 are merged, the three review defects and the admin price boundary are fixed, and the checklist is executed against a real Supabase staging schema.
- Report: `deliverables/gstack/pre-launch-check-albatal-store-2026-09-25.md`.

## New — 2026-09-24 (part 71: L2 remediation pass)

- Worktree: `.trees/fix-server-audit`; no commit, push, merge, or secret-value inspection performed.
- Client/server scope: forward migrations 068–073; admin money/RPC and projection contracts; order tracking projection; review RPC/storage/date/photo security; image mutation serialization and failure-safe reads; checkout bounds, positive totals, server metered rejection, strict money parsing, canonical metered display math, coupon idempotency, and response contracts; payment expiry/claim/proof integrity; rate-limit hardening; private proof buckets; nine-function deployment parity and secret preflights; re-runnable/transaction-wrapped checkout migration; staging verifier and migration generator corrections.
- Validation in the remediation worktree: analyzer **0 issues**; final full Flutter suite **1,132/1,132 passed**; final post-fix pricing/checkout/contract tests **23/23 passed**; Deno Edge tests **96/96**; migration parity **72 files/9 functions**; canonical and alternate bundles both contain **72 markers/2 reload notifies**; SQL dollar-quote balance **72/72**; formatter **568 files, 0 changed**; PowerShell syntax and `git diff --check` clean.
- Production cutover remains blocked on live Supabase schema/RLS/storage/payment verification, required secret configuration, and final integration review; all verifier findings, including the final Windows-secret and UUID cases, were addressed. No live provider, deployment, or secret-value test was performed.

## New — 2026-09-24 (part 70: comprehensive five-dimension L1 audit)

- Scope: current `master` @ `ead60c2`; 369 `lib/` Dart files, 197 `test/` Dart files, 68 committed Supabase migrations, 27 Edge Function TypeScript files, 47 scripts, 3 CI workflows, and project documentation. Pending `.trees` fix work were excluded from scoring; the only pre-existing root modification was this `STATE.md` record.
- Method: six parallel read-only specialist subagents (maintainability, clean architecture, code quality/testing, security, performance, adversarial release cross-check) with source-verified `file:line` evidence. No live Supabase, payment, deployment, or secret-value inspection was performed.
- Validation: CI-pinned analyzer **0 issues**; CI-pinned formatter **566 files, 0 changed**; Deno **93/93 passed**; exact-commit CI evidence **1,101/1,101 Flutter tests, 74.2% coverage**; `npm audit` **0 vulnerabilities**; `git diff --check` clean. Local Flutter test startup was blocked by the recorded cross-SDK native-assets kernel mismatch, not by a test failure; no local result is claimed.
- Scores (25/20/20/20/15 weighting): maintainability **6.8**, clean architecture **7.1**, code quality **5.6**, security **5.2**, performance **7.1**; weighted overall **6.4/10**.
- Critical current-HEAD findings: (1) admin money major/minor-unit mismatch can undercharge by 100x (`admin_product_edit_page.dart`, `admin_mappers.dart`, `admin_catalog_store.dart`); (2) sample/metered checkout is not implemented by migration 066 and can fail or charge incorrectly (`checkout_service.dart`, `cart_cubit.dart`, `066_coupon_checkout_and_validate_locked.sql`); (3) review submit commits a row then fails its post-insert `author_name` select, leaving hidden/orphaned state and retries blocked (`supabase_reviews_repository.dart`, `057_product_reviews.sql`); (4) customer review-photo upload targets the public product-images bucket but current storage policy is admin-only (`032_flash_sales_and_product_images.sql`); (5) stale generated `scripts/run_all_migrations.sql` ends at 013 and re-grants checkout to `PUBLIC`, omitting hardening through 067.
- Additional high-priority risks: InstaPay review/state race and method-switch stale payment rows (`041_instapay_payment_method.sql`); admin catalog/order/review API contract drift; checkout unbounded per-line transaction work and expiry backlog; deployment function-manifest drift; analytics/rate-limit ACL and proof-content validation gaps; public-WebView/backup/secret-rotation operational risks. These require server/migration review and are not auto-fixed in this L1 run.
- Release posture: **not release-ready for production cutover** until the financial/admin/review/payment contract defects and migration/deployment drift are fixed and verified against a real Supabase schema. Existing CI is useful client/unit evidence but does not execute SQL migrations or live RLS/payment flows.
- Top 5 remediation order: freeze production cutover; fix money units and catalog projections/RPC signatures; make checkout server-authoritative for sample/metered pricing; repair review storage/RLS and transactional submit; regenerate/publish migration and Edge Function deployment artifacts with CI SQL/ACL smoke tests. Then address payment state-machine hardening and performance ceilings.

## New — 2026-09-24 (part 69: approved protected audit patch)

- Owner explicitly approved protected Supabase/server, script, and CI changes. Worktree: `.trees/fix-server-audit`, branch `fix/server-audit`, baseline `ead60c2`; main source remains unchanged apart from this state record.
- Added forward migrations 068–072: extended/compatible admin RPC, private review submission/storage, claim-bound payment persistence and safe method transitions, hardened rate limiting, query indexes/admin sales aggregation, and bounded checkout wrapper with method/phone/item limits plus method-aware expiry.
- Updated client contracts for review RPC/signed private photos, canonical card-method handshake, server sales aggregate, and explicit metered/sample feature gating until server pricing semantics are approved. Regenerated `scripts/run_all_migrations.sql` with all 71 migrations.
- Hardened staging scripts to require TLS, regenerated schema verification, removed secret-scan allowlist weakness, made Android release depend on analyze/test, fixed daily-triage dependency setup, and added Android cleartext/backup protections.
- Verification: CI-pinned analyzer **0 issues**; formatter **568 files, 0 changed**; full Flutter **1,127/1,127 passed**; Deno **93/93 passed**; `git diff --check` clean. No local PostgreSQL/remote Supabase was available, so SQL was statically reviewed only.
- Verifier: **APPROVE**. Remaining owner action: review/apply migrations and deploy the coordinated client/server path; do not apply old combined SQL or push generated plugin noise. No commit, push, or merge performed.

## New — 2026-09-24 (part 68: L2 client audit fixes)

- Owner ask: “fix all issues” — treated as L2 approval for the client-safe batch. Worktree: `.trees/fix-audit-all`, branch `fix/audit-all`, baseline `ead60c2`; main checkout source was not changed.
- Implemented under `lib/` with focused tests: minor-unit-safe admin product/variant money boundaries; `payment_id` order tracking projection; complete product fabric/color projections; production coupon/analytics injection; fail-closed and serialized image mutations; awaited review submission and surfaced admin mutation failures; async `isClosed`/pagination guards; HTTPS-only zoom images; notification permission request; duplicate invoice implementation removal; CRLF-independent migration-contract test.
- Verification: CI-pinned Flutter 3.47.5 / Dart 3.13.4 analyzer **0 issues**; CI-pinned formatter **568 files, 0 changed**; full `flutter test --no-pub` **1,124/1,124 passed**; `git diff --check` clean; all 44 tracked + 3 untracked changes are under `lib/` or `test/`.
- Default Flutter 3.48 test startup remains blocked by the recorded cross-SDK native-assets kernel mismatch (expected kernel 130, found 138); this is toolchain/environmental, not a code failure.
- Verifier: **APPROVE**. No commit, push, or merge performed.
- Protected-path blockers intentionally not modified pending human review: `supabase/` RPC/RLS/review-storage/payment-claim/rate-limit/expiry changes; scripts with disabled TLS; stale combined migration artifact; CI/workflow changes; secrets and platform signing files. These require a separate reviewed patch.

## New — 2026-09-24 (part 67: comprehensive five-dimension L1 audit)

- Scope: current `master` @ `ead60c2`; 369 `lib/` Dart files, 197 `test/` Dart files, Supabase migrations/functions, scripts, CI, and project documentation.
- Method: six parallel read-only subagents plus an independent schema/contract cross-check. No source, migration, secret, or git-history changes. `git diff ea2b683..HEAD -- lib test` is empty.
- Evidence: analyzer 0 diagnostics, format clean, and CI 1,101/1,101 tests with 74.2% coverage are recorded from part 66; no live Supabase/Paymob calls were made. Local tests were not rerun because of the host-level Windows pipe-exhaustion incident.
- Scores: maintainability 7.5, clean architecture 6.8, code quality 5.9, security 6.0, performance 6.8. Weighted (25/20/20/20/15) = 6.64/10, reported as 6.6/10.
- Highest-risk confirmed findings: admin price minor/major-unit mismatch; payment-method/payment-row lifecycle mismatch; checkout server-contract gaps (coupon wiring, sample/metered handling, 15m vs 24h expiry); database/API contract drift (admin `tracking_number`, review table/view fields, catalog projections/RPC signature); incomplete server-owned review/payment/rate-limit authorization; admin image mutations are non-atomic.
- Standing items: `sbp_` token rotation and production cutover remain owner-only; audit is report-only and no L2 worktree was created.
...
- Owner ask: "continue" (autonomy grant stands). Triage first: the part-55 residual list is now fully closed — `cacheWidth`/`cacheExtent` budgets verified IN CODE (`related_card:36` 280, `wishlist_tile:36` 360, `image_gallery:90` 720, resolver plumbing), phone validator shipped in part 64, `compute` threshold-gated by design, server-side search ceiling is an owner decision.
- **Next open audit item — UX-019 (P1, still live):** `Money.format()` emitted `"1290 EGY"` at **29 call sites**. `EGY` is the ISO 3166 *country* code, not a currency; the codebase was internally inconsistent (`formatExact()` already defaulted to `EGP`, the admin console's `safeMinorToEgpLabel` prints `EGP`), and Arabic readers saw Latin-only prices.
- **Shipped:**
  - `Money.format()` default symbol `'EGY'` → **`'EGP'`** (ISO 4217), documented as the deterministic, locale-agnostic path (documents, admin tables, share text).
  - **NEW `lib/shared/l10n/money_copy.dart`** — `moneyText(l10n, Money)`: `intl` `NumberFormat.currency` with the ARB symbol (`currencySymbol`: en `EGP` / ar `ج.م.`). Two deliberate decisions, both documented in-file:
    1. **Placement pinned to the app's suffix convention** (`customPattern: '#,##0 ¤'`) instead of each locale's CLDR default — the Stitch system is built on "amount then currency", and CLDR's `en` default is a spaceless prefix (`EGP1,290`).
    2. **`ar` maps to `ar_EG` number data** — read from `intl-0.20.2/lib/number_symbols_data.dart`: plain `ar` is Latin-digit (`ZERO_DIGIT: '0'`, `GROUP_SEP: ','`), while `ar_EG` carries Arabic-Indic digits + `٬` + `٫`, i.e. exactly the audit's acceptance example. The store is Egypt-only by policy (EGP, Egyptian mobiles), so this is the honest reading of "Arabic" here.
    Decimals stay the app's rule, computed from `minorUnits` (whole → none, fractional → two), so a formatted price can never disagree with the amount the order records.
  - **All 25 money call sites migrated** (22 customer + 3 admin): `PriceText` now localizes; `related_card`, `metered_price_line`, `cart_summary` ×3, `cart_item_tile`, `add_to_cart_button`, `active_filters_bar`, `filter_sheet` ×5, `server_total_row`, `details_page` (WhatsApp share), `payment_method_page`, `instapay_instructions_page`, `stitch_product_grid_card` ×3 (incl. its semantics label), `stitch_flash_sale_card`, `stitch_hero_carousel`, `admin_order_tile`, `order_status_card`, `order_items_card`.
  - `StitchHeroSlide.fromProduct` now takes a **required** `l10n` — a context-free factory cannot reach `Localizations`, and a silent fallback would reintroduce the defect on the app's most prominent price.
- **Tests:** NEW `money_copy_test` (EN/AR contract + an explicit "no locale renders EGY" pin). `money_format_test` pins → `EGP`. All 46 `EGY` occurrences triaged: the two `invoice_pdf_money_test` assertions that require `EGY` to be ABSENT stay valid (the invoice is an EGP document). Two tests that computed expectations with the unlocalized `.format()` (`catalog_discount_label_test:109`, `home_text_scale_test:176`) now use `moneyText` — with grouping they would otherwise have disagreed with the widget ("1290 EGP" vs "1,290 EGP").
- ⚠️ **Host-level tooling incident (read before the next session):** from ~17:2x this machine exhausted its Windows pipe instances — `CreateFile failed 231 (All pipe instances are busy)` — so **every** Dart/Flutter command that spawns a child process failed: `flutter analyze`, `flutter test`, `dart analyze`, `flutter gen-l10n` (its crash report then crashed too), and even `dart run` (build hooks spawn `cmd.exe /c dart compile kernel`). Python's process spawning kept working, which is what made the workaround possible.
  - **Workaround:** drive the Dart analysis server directly over its own protocol from a Python client. Protocol gotchas: **string ids**, **no `jsonrpc` field** (it is not JSON-RPC 2.0 — the envelope is rejected with `INVALID_REQUEST`), **absolute file paths** (relative paths → `INVALID_FILE_PATH_FORMAT`), and `analysis.setAnalysisRoots` must COMPLETE before `analysis.getErrors` (else `GET_ERRORS_INVALID_FILE`). Tool kept at `~/.workbuddy-ai/tools/analyze_via_server.py`; usage: `python analyze_via_server.py lib test`.
  - **Evidence obtained this way:** format check (CI-pinned Dart 3.13.4) idempotent — 0 changed; analyzer — **0 diagnostics across 565 files** (lib + test), same server and `analysis_options.yaml` as `dart analyze`.
- **NOT verified locally: the test suite.** `flutter test` could not run in this session (incident above), so CI was the verifier — and it caught a real defect on the first run: **1,083 passed / 14 failed**, every failure the same shape (a whole amount printing `.00`).
  - **Root cause:** `NumberFormat.currency` applies its own fraction digits (default 2) and those **override the custom pattern's** `#,##0`, so `moneyText` rendered `1,290.00 EGP` for whole amounts. Grouping, suffix placement, the ISO code and the Arabic digits were all correct — only the decimal rule was wrong.
...
- Owner ask: "continue" (autonomy grant stands). Session targets were the audit's last residuals; two resolved by investigation (README links all resolve; voice-search mic absence is a documented decision), the third — the deferred "phone E.164 validator" (line 101 of the part-55-era list) — investigated to ground truth:
- **Evidence chain (why the validator never existed):** the client had NO phone input surface at all. Sign-up collects name/email/password only and auth metadata sends just `full_name` (`supabase_auth_repository.dart:39`); the profile page displays `phone` read-only; `AuthCubit.updateProfile` (the only client write path to `profiles.phone`) has zero UI callers; the `Address` entity had no phone field. `profiles.phone` is seeded only by the 003 trigger from `raw_user_meta_data->>'phone'`, which the app never sets — the column is effectively empty. A validator with no form to attach to would have been speculative dead code (same rationale as the voice-mic decision), so it was deferred, not forgotten.
- **UX-003 (UI_UX_AUDIT P0: "No phone number field" → COD orders undeliverable) unblocked by server review:** the audit gated the feature on "server snapshot schema — needs backend review". Review done by reading the RPC chain: `create_checkout_order` (4-arg 013→021→026, 5-arg 066) validates only `recipient`/`line`/`city` non-empty and stores `p_address` verbatim into `orders.address_snapshot` (JSONB) — an additive `phone` key is contract-safe with NO migration. Addresses are local-only (SharedPreferences via `LocalAddressRepository`) — no Supabase addresses table exists, so no schema change at all.
- **Implemented (client-only, end to end):**
  - `lib/core/utils/phone_validator.dart` (NEW) — shared Egyptian-mobile rule, `email_validator.dart` contract (top-level fn, `invalidMessage` param, null-on-valid). Accepts `01[0125]XXXXXXXX` (010/011/012/015 — the real allocation; the audit's `01XXXXXXXXX` sketch tightened on the third digit only), the 10-digit national form, and `+20`/`0020`/bare-20 country-code spellings, after stripping typed separators. Stores AS TYPED (courier dials what the customer sees; 065's `phone_digits` makes admin search separator-proof — no client reformat). Non-EG country codes rejected by policy (domestic store).
  - `Address.phone` entity field (default `''` — legacy rows have no phone on file) + `AddressCodec`: book shape 6→7 keys, snapshot 5→6 keys, decodes stay tolerant (missing phone → `''`); `order_codec.dart` picks the phone up automatically for local order records via `toJson`.
  - Both forms (the audit's "Address form ×2"): checkout bottom sheet `AddressForm` (phone right after name, `TextInputType.phone` + `AutofillHints.telephoneNumber`) and the addresses-book dialog. l10n: `phoneLabel`/`phoneInvalid` in en+ar, regenerated (512→514 keys/locale).
  - Tests: `phone_validator_test` (10 cases), codec shape pins updated (7-key/6-key + legacy tolerance + phone snapshot restore), form tests extended (invalid-phone blocks, separator-typed accepted, phone survives submission), dialog tests extended (EN/AR labels, 4×required + phone-invalid on empty submit, invalid-blocks-then-valid-saves). Suite: **1,090 passing** (was 1,074); analyze 0 under BOTH 3.13.4 (CI) and local 3.12.2; formatted with the CI-pinned Dart 3.13.4.
- **Latent bug fixed (found by the new test, not by review):** the addresses dialog was an inline `StatefulBuilder` whose controllers were disposed in a `finally` when `showDialog` resolved — i.e., at pop time, while the exit animation still rebuilt its TextFields. `A TextEditingController was used after being disposed` fired on every successful save; it had never surfaced because no widget test had ever saved that dialog successfully. Fixed at the root by extracting `_AddressDialog` as a StatefulWidget owning its controllers in `State.dispose()` (the `AddressForm` pattern), with `onSave` wired through the PAGE's context (dialog routes are siblings of `home`, so a dialog context cannot find the page's BlocProvider).
- Known scope note: legacy phone-less saved addresses still check out with `phone: ''` (server-tolerated); the audit's edit-address affordance can prompt completion later if the owner wants backfill.
- Human-only standing items unchanged: sbp_ token rotation, prod cutover of 066/067, delete `autonomy-probe-066@test.local`.
...
- Owner instruction: "full access to complete this project autonomously… proceed without waiting for confirmation." Treated as the L2/push/SQL-review gate for this session (prod remains untouched).
- **Baseline re-verified:** `flutter analyze --no-pub` → No issues found; `flutter test` → **1073/1073** (the `[SMOKE] FAIL admin_dashboard_data` line is the harness's intentional negative fixture).
- **Leftover OpenCode processes killed:** 7× `OpenCode.exe` found running (the part-61 .git-corruption source, flagged URGENT in OWNER_ACTIONS.md) — terminated via `Stop-Process` before any git work.
- **Premise proven LIVE before deploying (anon key / throwaway authenticated user via PostgREST):**
  - Issue 1: 5-arg `create_checkout_order` → `PGRST202` (no `p_coupon_code` param) — every coupon checkout broken on staging.
  - Issue 2: anon `validate_coupon` → HTTP 200 (open code-enumeration oracle).
  - Issue 6: `notifications` RLS OFF (anon insert reached column resolution, no RLS violation); `analytics_events` RLS already ON (anon insert → 42501).
- **Migrations applied to STAGING (zvpjngdgbpnkkqrorkul, `supabase db push --include-all`):**
  - `066_coupon_checkout_and_validate_lockdown.sql` — promoted from the 2026-09-23 reviewed proposal (docs/proposals/). 5-arg `create_checkout_order` with server-owned coupon resolution AFTER shipping+premium perk (invalid code ⇒ no discount, order still placed — 056 reviewer-note contract), `coupons_id`+`coupon_discount_minor` stamped on orders, response carries `coupon_discount_minor`. Old 4-arg overload kept for back-compat. `validate_coupon` locked to authenticated + throttled via 060 `rate_limit_take` (20/60s per user), return shape unchanged.
  - `067_lock_external_lineage_tables.sql` — `notifications`: RLS ENABLED + admin-read-only policy (`is_current_user_admin` from 061) + REVOKE anon/authenticated, GRANT SELECT authenticated (no client feature reads the table — grep-verified); `analytics_events`: RLS enable stated idempotently so prod converges.
  - Preceded by two comment-only stubs `61_/62_external_lineage_stub.sql` for the unpadded remote history rows `61`/`62` (external lineage, same precedent as 048–051) — required for `db push` to run; dry-run then listed exactly 066+067.
  - No dump backup possible (Docker Desktop engine never came up this session); rollback documented in each migration header — prior function bodies live verbatim in migrations 047/056.
- **Post-apply verification (all PASS, same probes as pre-apply):**
  1. anon `validate_coupon` → `42501 permission denied` (was 200)
  2. anon GET `notifications` → `42501 permission denied` (was 200 [])
  3. anon INSERT `analytics_events` → 42501 RLS (unchanged)
  4. authenticated 5-arg checkout → `P0001 'Variant not found'` domain error (was PGRST202 — signature now resolves)
  5. authenticated `validate_coupon` → 200
  6. throttle: calls 1–19 OK, call 20+ → `Too many coupon checks`
  - Residue: throwaway user `autonomy-probe-066@test.local` on staging (no admin/service key to delete; harmless). No orders/variants/coupons mutated (probes stop at domain validation).
- **OWNER_ACTIONS.md executed:** Issues 1/2/6 closed (staging), OpenCode kill done, untracked docs committed (secrets-swept: no sbp_/JWT/conn-strings), app_router review moot (shipped in 3a03229), CI pin resolved-by-documentation (3.47.x stable kept; skew documented in loop-constraints.md), push gate executed. **Only remaining human item: Issue 27 sbp_ token rotation (provider dashboard).**
- `.flutter` + `.flutter_tool_state` gitignored (transient tool state).
- **lib/ code unchanged this part** — one TEST updated: `supabase_rpc_contract_test.dart` pinned the newest `create_checkout_order` definition to 047; since 066 rewrites it (047 body verbatim + coupon block), the pin moved to 066 and the grant-posture assertion now checks the 5-arg identity. Added a new contract test `coupon discount is server-owned (migration 066 contract)` pinning: coupon resolves after shipping+premium, invalid code ⇒ no-discount (no exception), floor at zero, `coupons_id`+`coupon_discount_minor` stamped, `validate_coupon` anon-revoked + throttled. Commit `907fcb1` then test fix — final gates: analyze 0, **1074/1074** (02:44).
- **No lib/ code changed this part** — the app's own gates were re-certified at baseline; migrations are DB-side.

## New — 2026-09-23 (part 61: L2 fix-all wave — owner-approved push executed; corruption incident + recovery)

- Owner instruction: "L2 enable, fix all, and if step needs me, collect them in an MD file." Executed across 6 waves in worktree `.trees/l2-full` (branch `fix/l2-full` @ `e2f3ef4`, ~90 files dirty/staged).
- Waves: (1) orders CSV exporter domain→data + checkout port retyped `Map`→`Address?` (5-key wire shape moved to `CheckoutService` via `AddressCodec`) + `OrderStatus` enum on `PendingOrder`; (2) `CatalogPriceBounds` rename split, curated `colorName` matching, new `catalog_color_label.dart` + 11 EN/AR keys + `gen-l10n`; (3) perf/quality: `ListView.builder`, `AppDurations.searchDebounce`, `buildWhen`, `Expando` memo, exhaustive payment dispatch, dual-cache image fix (`detailImageWidth` + bounded decode), wishlist resolve out of `build()`; (4) shared `auth_failure_listener` + `catalogStatusGuard`, address entity to `domain/entities/`; (5) `INSTRUCTIONS.md` architecture rules + new color tests; (6) analyze→fix loop to zero.
- **Final gates (worktree, at approval time):** `flutter analyze --no-pub` → **No issues found**; `flutter test` → **+1058 All tests passed** (incl. `[SMOKE] PASS` harness; new color tests included). Local `dart format` stays waived (Dart 3.12.2 vs CI pin — `loop-constraints.md`).
- **Owner queue:** `OWNER_ACTIONS.md` at repo root — coupon `p_coupon_code` migration (P1 money-path), `validate_coupon` REVOKE-from-anon + throttle applied check (P1), `sbp_` token rotation, RLS parity (066/067), commit untracked audit/delivery docs, CI Dart-pin alignment.
- **Push gate executed (owner approved "Merge + push" via session decision, 2026-09-23):** 90 dirty worktree files → 71 committed as `38a74b6` on `fix/l2-full` (12 transient gate logs left untracked; 6 generated plugin registrants were CRLF-only noise, normalized out). Includes the `app_router`→`auth_redirect`+`route_pages` split — owner approval covered it.
- **INCIDENT — concurrent OpenCode sessions corrupted `.git` (no source files lost):** 5× `OpenCode.exe` from prior loop sessions were live; during the merge they deleted `.git/refs/` and pruned ALL loose objects. Lost as objects: the wave commit `38a74b6`, the 8 prior wave commits (incl. widget-separation `f786804`), plus unrelated loose objects. Fresh nested ref writes (`refs/heads/fix/*`) were being deleted within seconds. `.git/refs/` was restored from `ORIG_HEAD`/`FETCH_HEAD` (master, origin/master); all other branch tips resolve from `packed-refs`.
- **RECOVERY:** `.trees/l2-full` file tree (full branch-tip content of all 9 wave commits) was intact on disk. Re-applied as a single squashed commit **`3a03229`** — "fix(l2): L2 fix-all wave — analyze 108→0…" (211 files, +8237/−5892) directly on `master` — content-equivalent to squash-merging `fix/l2-full`. Pushed to `origin/master`. `fix/l2-full` retired (dangling ref deleted after its object was pruned).
- **History note:** wave-commit SHAs from parts 59–61 no longer resolve (objects pruned); the granular 9-commit history is flattened into `3a03229`. Per-wave verifier APPROVE evidence remains in the records below.
- **Post-push verification caught residual damage (2026-09-23):** gates re-run on the merged master exposed one more daemon revert that the worktree's broken git status had masked — `lib/core/entities/profile.dart` regressed to its pre-part-57 state, breaking `profile_test.dart` compilation (8 analyze errors; 1 of 1067 tests failed to load). Neither file is in the wave's changed set. `profile.dart` restored verbatim from `ccd66c7`, committed as **`86f7c2b`**, pushed (`3a03229..86f7c2b`).
- **Final gates on repaired master:** `flutter analyze --no-pub` → **No issues found**; `flutter test` → **+1073 All tests passed** (3:59, zero errors). `origin/master = 86f7c2b` confirmed via `ls-remote`; stale local tracking ref repaired.
- **Housekeeping:** dead worktrees `.trees/fix-audit-full` + `.trees/fix-core-cleanup` moved to `.trees/_quarantine-2026-09-23/` (reversible; content merged per part 58); worktree admin metadata already dissolved from `.git/worktrees/`. `.trees/l2-full` move pending — an OpenCode process holds a handle on it ("Device or resource busy").
- **New owner actions:** (a) close/kill the 5 `OpenCode.exe` processes before any further git work in this repo — they race agent `.git` writes; (b) after verifying this push, delete dead worktrees `.trees/l2-full` (git-link dead: "not a git repository: (NULL)") and the pre-broken `.trees/fix-audit-full` / `.trees/fix-core-cleanup` (`0000000`) via `git worktree prune` + dir removal.
- **No records lost:** part 59 (census) + both part-60 records preserved below (this session's + the branch's widget-separation record).

## New — 2026-09-22 (part 60: widget separation, verifier APPROVE, committed locally)

- Branch `fix/widget-separation` in worktree `.trees/fix-widget-separation`, off `ef91836`. 36 modified + 84 new files, **all under `lib/`** (the only non-`lib/` diff entries are 7 Flutter-generated plugin registrants with zero content change — pure CRLF artifacts). No `pubspec.yaml`, CI, `supabase/` migrations, `.env`/secrets touched.
- **Gates (re-run independently by the verifier):** `flutter analyze --no-pub` — **No issues found**; `flutter test --no-pub` — **1044/1044 passed**. Verifier verdict **APPROVE**: moved bodies byte-identical vs HEAD, no string literal / error code / query shape / default changed, all re-export shims present.
- **Largest remaining files:** `app_router.dart` 374 (deferred, human-review gate on GoRouter restructuring) and generated `lib/generated/l10n/*` (never split). Next largest non-generated: `paymob_payment_service.dart` 299, `checkout_cubit.dart` 298, `details_page.dart` 295.
- Extractions this run: `admin_customer_search.dart` wired (duplicate `CustomerCursor` typedef dropped in favour of `admin_customers_port.dart`; `supabase_admin_repository.dart` 825→241 as a facade over six port-aligned workers `admin_{orders,sales,catalog,coupons,customers,reviews}_store.dart`, each `implements` its narrow port); `storefront_persistence.dart` 305→~190 via `order_codec.dart`; `supabase_catalog_repository.dart` 421→~230 via `catalog_persistent_cache.dart` + `catalog_page_fetch.dart` + `ProductCodec.listFromRows`/`productSelect`; `paymob_payment_service.dart` 358→299 via `paymob_failures.dart`; `admin_mappers.dart` 312→~180 via `admin_sales_mappers.dart` (facade forwards); `auth_cubit.dart` 303→~235 via `parts/auth_snapshot_wipe.dart`; `instapay_instructions_page.dart` 430→~270 via `widgets/instapay_{transfer_card,proof_form,error_body,proof_picker}.dart`; `admin_product_edit_page.dart` 423→~220 via `widgets/product_edit_{form,loaders}.dart`.
- Fixed along the way: wrong import depth in `admin_image_grid.dart` (`../../../../shared` → `../../../../../shared`) that broke test compilation; two unused imports (`result.dart` in `admin_image_manager_page.dart`, `build_context_x.dart` in `admin_detail_row.dart`); `export` before `part` ordering error in `auth_cubit.dart`; accidentally-dropped `paymobTimeoutFailure` helper during an edit; `payment_start_failed` code preserved on the initiate-failure path (the generic `network_error` would have changed behavior).
- Verifier cosmetic note (not a rejection ground): `fetchCatalogPage` returns an unused `truncated` field — the truncation warning still logs on the same condition; the field is dead, not a behavior change.

## New — 2026-09-22 (part 59: L1 file-size / multi-widget census, report-only)

- Owner ask: "explore codebase, search all files, if file includes more than one widget make separation, or reusable component, reduce all files to max 200-300 lines". L1 report-only per AGENTS.md — no source edits, no worktree, no gates run.
- Census method: `Get-ChildItem -Recurse *.dart lib` + `class\s+\w+\s+extends\s+(Stateless|Stateful|Consumer)` regex; line counts via `Measure-Object -Line`. Generated l10n excluded from action list.
- Scale: 269 dart files in `lib/`; 41 files >200 lines (38 non-generated + 3 generated); 18 files >300 lines (15 non-generated + 3 generated `app_localizations*.dart` 2858/1294/1281 — generated, never split by hand).
- The 15 non-generated >300-line files (must-split to hit ≤300): `supabase_admin_repository:781`, `checkout_page:470`, `home_page:458`, `admin_product_edit_page:409`, `instapay_instructions_page:407`, `admin_image_manager_page:404`, `payment_cubit:402`, `supabase_catalog_repository:381`, `app_router:366`, `reviews_section:362`, `stitch_hero_carousel:355`, `settings_page:342`, `paymob_payment_service:328`, `auth_cubit:325`, `payment_method_page:316`.
- The 200-300 band (23 files, split on next pass): `order_detail_cards:297`, `admin_mappers:287`, `storefront_persistence:285`, `details_page:282`, `admin_variant_editor:278`, `checkout_cubit:276`, `admin_order_detail:260`, `catalog_state:255`, `admin_customers_cubit:253`, `onboarding_page:252`, `catalog_page:251`, `admin_customers_page:244`, `app:242`, `app_lock_gate:239`, `product_mapper:238`, `admin_cubit:237`, `supabase_auth_repository:227`, `sign_in_page:225`, `catalog_cubit:218`, `admin_inventory:211`, `product_details_cubit:205`, `service_locator:203`, `storage_service:202`.
- Multi-widget files (26 with ≥2 widget classes — separation candidates): `order_detail_cards:7` (OrderStatusCard, CustomerCard, OrderItemsCard, DeliveryAddressCard, FulfillmentActions, _ActionTile, _DetailRow) · `reviews_section:6-7` (ReviewsSection, _ReviewsView, _ReviewTile, _ReviewTileBody, _ReviewsInlineList, _ReviewSubmitSheet+State) · `skeleton_loaders:6` (CatalogSkeleton, _ProductCardSkeleton, OrdersSkeleton, _StaticCardSkeleton, _StaticTileSkeleton, _OrderTileSkeleton) · `checkout_page:5-6` (CheckoutPage + _CouponCard+State, _ShippingAddressCard, _ServerTotalsCard, _ServerTotalRow) · `home_page:4-5` (HomePage+State, _SectionHeader, _FlashSaleCard, _PopularHeader) · `settings_page:4` (SettingsPage + _DeleteAccountTile, _NotificationToggleTile, _AppLockToggleTile+State) · `stitch_hero_carousel:4-6` (StitchHeroSlide, StitchHeroCarousel+State, _Dot, _HeroSlideCard, _GoldPill) · `paymob_checkout:3`, `profile_page:3` (_GuestProfile, _AuthenticatedProfile), `admin_dashboard:3` (+_StatCard, _ActionTile), `order_status_timeline:3` (+_StepRow, _CancelledTerminal) · 2-widget: `app_lock_gate` (+_AppLockScreen), `stitch_product_grid_card`, `admin_catalog`, `order_success`, `variant_selector` (+_MeteredPriceLine), `admin_inventory` (+_StockTile), `admin_orders`, `admin_image_manager` (+_IconBtn), `admin_categories`, `admin_coupons`, `payment_method` (+_PaymentOption), `categories_page`, `onboarding` (+_OnboardingStepView, _OnboardingStep), `admin_reviews`, `admin_sales_dashboard`, `forgot_password`, `wishlist_page`, `admin_product_edit` (page+State only — split by sections, not widgets).
- Reusable-component wins (same private widget shape repeated): `_ActionTile` exists in BOTH `admin_dashboard_page` and `order_detail_cards` → one shared `AdminActionTile`; `_SectionHeader`/`_PopularHeader`/`_StatCard` → shared section-header/stat cards; `_IconBtn`/`_StockTile`/`_PaymentOption`/`_ServerTotalRow`/`_DetailRow` → shared list-row/button atoms; `skeleton_loaders` 6× → one file per skeleton already small (152) — keep file, just extract each skeleton to its own file only if reused; `stitch_hero_carousel` _Dot/_GoldPill → shared stitch atoms.
- Non-widget splits (no widget count but over budget): `supabase_admin_repository:781` → split impl by the 6 narrow ports already created in part 55/56 (orders/coupons/customers/reviews/sales/catalog); `supabase_catalog_repository:381` → queries vs mappers vs persistence; `app_router:366` → per-feature route lists (P4 residual); `payment_cubit:402`/`auth_cubit:325`/`checkout_cubit:276`/`admin_cubit:237` → state/mixin per payment-method or auth-flow; `admin_mappers:287`/`product_mapper:238` → one mapper file per entity; `service_locator:203`/`storage_service:202` → per-feature registration modules.
- Proposed L2 batches (needs explicit human L2 enable + worktree per AGENTS.md, `lib/` only): B1 `checkout_page` → `widgets/checkout/{coupon_card,address_card,totals_card}.dart`; B2 `home_page` → `widgets/home/{section_header,flash_sale_card,popular_header}.dart`; B3 `reviews_section` → `widgets/reviews/{review_tile,submit_sheet,inline_list}.dart`; B4 `order_detail_cards` → 5 single-card files + shared `detail_row.dart`; B5 `settings_page` → `widgets/settings/*_tile.dart`; B6 `stitch_hero_carousel` → `hero_slide_card.dart` + `hero_dots.dart`; B7 repos/cubits/router as above. Each batch: extract with barrel exports so imports don't churn, then `flutter analyze` + `flutter test`, verifier sub-agent.
- No code touched, no commit, no push. Standing items unchanged: `sbp_` token rotation; remote `origin/fix/audit-findings-0919` delete (owner).
...
- Worktree `.trees/fix-core-cleanup` (`fix/core-cleanup` @ master `00f0553`, 4 lib + 1 test): `money.dart` — deleted dead `majorLabel()`/`egpLabel()`; `address_codec.dart` — `fromOrderJson` delegates to `fromJson` (was byte-identical); `profile.dart` — `fromRow` via `safe_parse`, missing/mistyped `id` throws `FormatException` (fail-closed through `Result.guard`); `variant_selector.dart:82` — stale `egpLabel` comment reworded; `profile_test.dart` — 2 new pins (mistyped-column degrade, missing-id `FormatException`).
- **Correction recorded (would-have-been-wrong):** L1 claimed `Money.format()` was dead — grep proved it has ~20 UI call sites pinned by tests (`money_format_test`, `cut_length_pricing_test` expect `'1290 EGY'`), so the `'EGY'` default is load-bearing copy and `format()` was kept; only the two truly caller-less helpers went.
- `lib/core/helper/` needed no deletion — empty dir was never tracked in git (absent from fresh worktree checkout).
- **Evidence:** `flutter analyze --no-pub` — **No issues found**; full `flutter test` — **1032/1032** (1030 baseline + 2 new); post-comment-edit targeted — **50/50**. Verifier: **APPROVE**. Platform `generated_*` churn reverted; diff is lib/+test/ only. No commit/push — not requested.

## New — 2026-09-21 (part 56: L2 top-up gates green + verifier APPROVE, uncommitted on fix/audit-top5-full)

- Worktree `.trees/fix-audit-full` (`fix/audit-top5-full` @ `c7039f3` + 23 lib / 5 test / 1 new-test uncommitted): P3 — reviews repo all-Failures carry `code:` + `maxReviewTextLength=2000` fail-fast guards, `reviews_cubit` classifies on `error.code ?? error.message`, OAuth coded errors, sign-in code-first check, settings `failureText(code:)`, checkout `_userMessageForPostgrest` returns `(message, code)` always `kCheckoutFailedCode`. P5 — `Log.w('...', error: e)` sweep, storage `..` debug asserts. P6 — explicit selects (`profile:25`, `admin variants:598`), catalog JSON via `compute`.
- **Gates caught a real regression (attempt 1→2):** unconditional `compute(jsonDecode)` on the offline restore path broke `offline_catalog_pages_test` cold-start (isolate spawn outlasts pumped fake-time; isolated rerun hung the pipe). Fix (attempt 2, lib-only): threshold-gated compute — inline ≤64KB chars / ≤40 items, isolate above (`_isolateJsonChars`, `_isolateItemBudget`) — faster real cold start for small caches too. Test file NOT weakened (same asserts, no extra pumps).
- **Evidence (worktree):** `flutter analyze --no-pub` — **No issues found**; targeted 7 files — **37/37 pass**; full `flutter test` — **1042/1042 pass** (baseline 1034 + 8 new). P4 verified closed: all 96 `getIt`/`sl<` hits in `bootstrap`/`app`/`service_locator`/`app_router` (composition root), zero view-layer. Verifier sub-agent: **APPROVE** (scope clean, no leaks, async conversion safe).
- Housekeeping: reverted `flutter pub get` side-effect churn in `linux|macos|windows/generated_*` (CRLF-only) to keep diff lib/+test/ only. No commit/push — owner-gated.
- Deferred residuals (polish, no regression risk taken): phone E.164 validator, server-side search vs 100-row ceiling (owner decision, `Log.w` already warns when page full), gallery/related/wishlist `cacheWidth`, `compute` for tiny `storefront_persistence`/`recently_viewed` payloads (inline correct per threshold rationale).

## New — 2026-09-21 (part 55: full 5-dimension L1 audit, 5 parallel subagents, read-only)

- Scores: Maintainability **7.0** (262 lib files, 8 features × data/domain/presentation, 0 TODO, en=512/ar=512 parity; docked: `supabase_admin_repository:812` lines, `checkout_page:490`, `home_page:475`, `payment_cubit:443`, 30× `EdgeInsets.all(16)` vs tokens) · Clean Arch **6.5** (zero widget→Supabase leakage, `Result<T>` boundary, mappers in data; docked: 21-method `AdminRepository`, 6 `?? getIt<>()` view fallbacks, 1 use-case vs ~15 direct repo calls) · Quality **6.5** (analyze 0, zero `print`, `failureText(code:)` ×29; docked: P0 wrong-label trio, hardcoded `CatalogSortLabel`/sort + `statusLabel`, message-matched codes) · Security **8.5** (no criticals; secrets/Keystore/server-money/RLS + gitleaks CI strong; residual: `app_image:49` HTTP accept, `Log.w($e)` interpolation, missing phone/review-text guards) · Performance **7.5** (420/720 render widths, 5-min TTL + O(1) maps, rebuild isolation; docked: gallery/related/wishlist decode overshoot, client-side search over 100-row ceiling, UI-thread JSON).
- Overall **7.2/10** (equal-weight mean 36.0/5; weighted 25/20/20/20/15 = 7.18). Method: 5 parallel read-only subagents, evidence at file:line; no code touched, no push.
- Top-5 carried to next L2 gate (owner must enable L2): P1 wrong-label trio → single `orderStatusLabel()`; P2 AR-broken sort/status literals → ARB keys; P3 message-matched codes (`reviews_cubit:74`, `sign_in_page:221`, `checkout_service:135`, `settings_page:272` verbatim) → `error.code` + `failureText(code:)`; P4 arch (`admin_repository:35-204` split by bounded context + remove 6 view `getIt` fallbacks + per-feature routes); P5 security hardening (`app_image:49` HTTPS-only, `Log.w(error:e)` safe path, phone E.164 + review-text guards). Perf runner-up P6: gallery `cacheWidth:720` + related ~280/wishlist ~360 + server-side search to lift 100-row ceiling.
- Standing items unchanged: `sbp_` token rotation; remote `origin/fix/audit-findings-0919` delete (owner); untracked `.cluster/`/`delivery/` provenance check.
> Merge note (2026-09-21): below are the inherited `c7039f3` session records (its own parts 56/55) preserved from `fix/audit-top5-full`; the merged top-up is recorded in part 56 above.
