...
void main() {
  group('COD success path', () {
    test('calls confirmCodPayment with the order ID', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-cod-1');
    ...
    test('emits success with server transaction ID on server success',
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-cod-2');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
    ...
    test('emits failed with server error on COD rejection', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-cod-3');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
    ...
    test('does not emit client-generated transaction ID', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-cod-5');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
    ...
    test('amount is preserved from server-computed init (not overwritten)',
      ...
      cubit.initPayment(amount: const Money.egp(500), orderId: 'ord-cod-amt');
      expect(cubit.state.amount, const Money.egp(500));
  ...
  group('COD already_confirmed path', () {
    test('server returns ok:true code:already_confirmed → success', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-idem-1');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
    ...
    test('idempotent re-confirmation is safe via server', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-idem-2');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
  ...
  group('COD timeout path', () {
    test('emits failed when RPC hangs and timeout fires', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-timeout');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
  ...
  group('COD failure path', () {
    test('emits failed on network error', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-cod-4');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
    ...
    test('emits failed and forwards unknown error code unchanged', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-unknown');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
  ...
  group('COD non-owner rejection', () {
    test('server returns not_owner → failed with safe code', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-not-owner');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
  ...
  group('COD non-COD rejection', () {
    test('server returns payment_not_cod → failed with safe code', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-not-cod');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
  ...
  group('COD cancelled order rejection', () {
    test('server returns order_not_pending → failed with safe code', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-cancelled');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
      await cubit.processPayment(customerEmail: 'a@b.c');
  ...
  group('Double-call guard', () {
    test('processPayment ignores re-entry during processing', () async {
      ...
      cubit.initPayment(amount: const Money.egp(100), orderId: 'ord-double');
      cubit.selectMethod(PaymentMethod.cashOnDelivery);
