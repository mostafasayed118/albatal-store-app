...
-- Runner fix: _r() executes while role is switched to anon/authenticated,
-- which otherwise cannot INSERT into this postgres-owned temp table.
-- Transaction-scoped grant; the temp table dies at ROLLBACK.
GRANT SELECT, INSERT ON _rls_results TO PUBLIC;

-- Helper to record a result.
CREATE OR REPLACE FUNCTION _r(test_id TEXT, p_desc TEXT, expected TEXT, actual TEXT)
RETURNS VOID LANGUAGE plpgsql AS $$
BEGIN
  INSERT INTO _rls_results VALUES (test_id, p_desc, expected, actual,
...
-- ============================================================
-- Runner fix: service_role lacks INSERT on auth.users; seed as the
-- session's login role instead. Everything is rolled back at the end.
RESET ROLE;
...
SELECT _r('1.1', 'anon cannot read profiles', '0', (SELECT count(*)::text FROM profiles));
-- 1.2 Cannot read orders
SELECT _r('1.2', 'anon cannot read orders', '0', (SELECT count(*)::text FROM orders));
-- 1.3 Cannot read order_items
SELECT _r('1.3', 'anon cannot read order_items', '0', (SELECT count(*)::text FROM order_items));
-- 1.4 Cannot read addresses
SELECT _r('1.4', 'anon cannot read addresses', '0', (SELECT count(*)::text FROM addresses));
-- 1.5 Cannot read cart_items
SELECT _r('1.5', 'anon cannot read cart_items', '0', (SELECT count(*)::text FROM cart_items));
-- 1.6 Cannot read wishlists
SELECT _r('1.6', 'anon cannot read wishlists', '0', (SELECT count(*)::text FROM wishlists));
-- 1.7 Cannot read payments
SELECT _r('1.7', 'anon cannot read payments', '0', (SELECT count(*)::text FROM payments));
-- 1.8 CAN read public catalog (products)
SELECT _r('1.8', 'anon can read products', '>=1', (SELECT CASE WHEN count(*) >= 1 THEN '>=1' ELSE '0' END::text FROM products));
-- 1.9 CAN read public catalog (categories)
SELECT _r('1.9', 'anon can read categories', '>=1', (SELECT CASE WHEN count(*) >= 1 THEN '>=1' ELSE '0' END::text FROM categories));
-- 1.10 CAN read public catalog (product_variants)
SELECT _r('1.10', 'anon can read product_variants', '>=1', (SELECT CASE WHEN count(*) >= 1 THEN '>=1' ELSE '0' END::text FROM product_variants));
-- 1.11 CAN read public catalog (product_images)
SELECT _r('1.11', 'anon can read product_images', '>=0', (SELECT CASE WHEN count(*) >= 0 THEN '>=0' ELSE '-1' END::text FROM product_images));
-- 1.12 Cannot read notifications
SELECT _r('1.12', 'anon cannot read notifications', '0', (SELECT count(*)::text FROM notifications));
-- 1.13 Cannot read analytics_events
SELECT _r('1.13', 'anon cannot read analytics_events', '0', (SELECT count(*)::text FROM analytics_events));
-- 1.14 Cannot read error_logs
SELECT _r('1.14', 'anon cannot read error_logs', '0', (SELECT count(*)::text FROM error_logs));
...
SELECT _r('2.14', 'user_a cannot read user_b order_items', '0',
...
SELECT _as('22222222-2222-2222-2222-222222222222'); -- user B, non-admin
...
END $$;
SELECT _service();
SELECT _r('3.1', 'non-admin cannot INSERT products', '0',
...
END $$;
SELECT _service();
SELECT _r('3.2', 'non-admin cannot UPDATE products', 'RLS Test Product',
...
END $$;
SELECT _service();
SELECT _r('3.3', 'non-admin cannot DELETE products', '1',
...
END $$;
SELECT _service();
SELECT _r('3.4', 'non-admin cannot INSERT categories', '0',
...
END $$;
SELECT _service();
SELECT _r('3.5', 'non-admin cannot UPDATE categories', 'RLS Test Cat',
...
END $$;
SELECT _service();
SELECT _r('3.7', 'non-admin cannot set is_admin=true', 'false',
...
SELECT _as('11111111-1111-1111-1111-111111111111'); -- user A
...
SELECT _as('11111111-1111-1111-1111-111111111111');
SELECT _r('4.1', 'authenticated cannot INSERT payments directly', '0',
...
END $$;
SELECT _service();
SELECT _r('4.4', 'authenticated cannot UPDATE payments', 'pending',
...
END $$;
SELECT _service();
SELECT _r('4.5', 'authenticated cannot UPDATE orders directly', 'pending',
...
BEGIN
  BEGIN
    ...
    VALUES ('11111111-1111-1111-1111-111111111111', 'pending', 1, 0, 1,
      'fraud', '{}'::jsonb);
  -- Runner fix: RLS denial raises insufficient_privilege (42501), not check_violation
  EXCEPTION WHEN insufficient_privilege OR check_violation THEN NULL;
  END;
END $$;
SELECT _service();
SELECT _r('4.6', 'authenticated cannot INSERT orders directly', '0',
...
END $$;
SELECT _service();
SELECT _r('4.7', 'authenticated cannot INSERT order_items directly', '1',
